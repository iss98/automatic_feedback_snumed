...
st.markdown("\left ( 2^4 right )^x times \left ( 2^2 right )^x = 2^3 times 2^{ 3x } **=** 2^{ 4x } times 2^{ 2x } = 2^3 times 2^{ 3x } **=** 2^{ 3x } = 2^3")
st.write("2. 식오류 : 곱셈 기호를 덧셈 기호로 혼동하여 작성함, 중간에 옮겨적는 과정에서 수나 문자를 잘못 적음, 지수에 있는 미지수를 빼고 계산함")
st.markdown("<strong> 2^4 times 2^2 = 2^3 times 2^3 = 16 times 4 = 8 times 8</strong>  x = 1")
st.write("3. 이항오개념 : 등식의 성질을 이용하여 양변에 2^{3x}를 나눈 것인데 이항이라는 용어로 표현함")
st.markdown("\left { 2^6 right }^x = 2^3 times 2^{ 3x }  <strong>이항</strong>  2^{ 3x } = 2^3")
